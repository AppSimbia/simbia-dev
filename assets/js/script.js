// TROCAR LOGO E DESTACAR MENU COM BASE NA SEÇÃO
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!headerLogo || sections.length === 0) return;
  
    // Seções com fundo claro
    const lightSections = ['about-02s1', 'about-02s2', 'features-04','team-06'];
  
    // Caminhos das logos
    const logoDark = 'assets/images/logos/icon-simbia-escrito-escuro.svg';
    const logoLight = 'assets/images/logos/icon-simbia-escrito-claro.svg';
  
    // Transição suave da logo
    ...
    // Verifica qual seção está visível no topo
    function checkVisibleSection() {
      ...
      for (let section of sections) {
        ...
        if (rect.top <= headerHeight && rect.bottom > headerHeight) {
          ...
          if (lightSections.includes(activeSectionId)) {
            changeLogo(logoDark);
          } else {
      ...
      if (activeSectionId) {
        navAnchors.forEach(anchor => {
          ...
          if (href && href.includes(`#${activeSectionId}`)) {
    ...
    window.addEventListener('scroll', () => {
      ...
      scrollTimeout = setTimeout(checkVisibleSection, 20);
    });
